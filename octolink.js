...
// @version      4.2.0
// @description  Lõi bypass: giao thức 4 chặng /check/continue với LIVE CORE từ octolink.vip. Nạp bởi loader.user.js.
...
(function () {
  ...
  function matchHost(h, d) {
    return h === d || (h.length > d.length && h.slice(-(d.length + 1)) === '.' + d);
  }
  
  var isOcto = matchHost(host, 'octolink.vip');
  var isGuideHost = matchHost(host, 'linkhuongdan.online') || 
                    matchHost(host, 'totreview.com') || 
                    matchHost(host, 'minuc.vn');
  ...
  if (isFinish && !is404Page()) {
    console.log('[otl-bypass] Trang captcha — tự tắt.');
    return;
  }
  ...
  // =====================================================================
  function gm(n) {
    return (typeof GM !== 'undefined' && GM && typeof GM[n] === 'function') ? GM[n].bind(GM) : undefined;
  }
  ...
  function safeRequest(opts) {
    ...
    var onErr = opts.onerror;
    var onTmo = opts.ontimeout;
    ...
    function fallback() {
      ...
      if (opts.headers) {
        for (var k in opts.headers) {
          fOpts.headers[k] = opts.headers[k];
        }
      }
      ...
      window.fetch(opts.url, fOpts).then(function (resp) {
        return resp.text().then(function (txt) {
            ...
            responseText: txt,
            response: txt,
            responseHeaders: Array.from(resp.headers.entries()).map(function (h) {
              return h[0] + ': ' + h[1];
            }).join('\n')
          });
        });
  ...
  var SHIM_SCRIPT = '(function(){try{Object.defineProperty(window,"top",{get:function(){return window;},configurable:true});}catch(e){}try{Object.defineProperty(window,"parent",{get:function(){return window;},configurable:true});}catch(e){}try{Object.defineProperty(window,"frameElement",{get:function(){return null;},configurable:true});}catch(e){}try{Object.defineProperty(window,"opener",{get:function(){return null;},configurable:true});}catch(e){}try{Object.defineProperty(document,"hidden",{get:function(){return false;},configurable:true});}catch(e){}try{Object.defineProperty(document,"visibilityState",{get:function(){return "visible";},configurable:true});}catch(e){}try{Object.defineProperty(document,"hasFocus",{value:function(){return true;},configurable:true});}catch(e){}})();';
  ...
  function initUI() {
    ...
    UI = {
      ...
      status: function (text, mode) {
        UI.stt.textContent = text;
        UI.dot.className = 'oc-dot' + (mode ? ' ' + mode : '');
      },
      setRail: function (pct) {
        UI.rail.style.width = Math.max(0, Math.min(100, pct)) + '%';
      },
    ...
    p.querySelector('#ocMin').onclick = function () {
      p.classList.toggle('oc-collapsed');
    };
    p.querySelector('#ocX').onclick = function () {
  ...
  function notify(t) {
    try {
      if (gNoti) gNoti({ title: 'OctoLink Bypass', text: t, timeout: 4000 });
    } catch (e) {}
  }
  ...
  function readJsConfig(text) {
    ...
    function bool(name) {
      var m = text.match(new RegExp('var\\s+' + name + '\\s*=\\s*(true|false)'));
  ...
  function probeCore(w) {
    try {
      if (typeof w.__hf === 'function') {
        ...
        if (sig && typeof sig['x-guard-sig'] === 'string' && sig['x-guard-sig'].length > 0) {
          lastProbe = 'ready';
          return 'ready';
        }
        lastProbe = 'có __hf nhưng sig hỏng';
        return 'bad-sig';
      }
      lastProbe = 'chưa có __hf';
    } catch (e) {
      lastProbe = 'probe lỗi: ' + (e && e.message ? e.message : e);
    }
    return 'loading';
  }
  ...
  function diagCore(w) {
    ...
    ['__hf', '__b110671', '__se4ce', '__se331'].forEach(function (n) {
      try {
        if (typeof w[n] !== 'function') miss.push(n);
      } catch (e) {
        miss.push(n);
      }
    });
  ...
  function bootLiveCore(done) {
    ...
    safeRequest({
      ...
      onload: function (resp) {
        ...
        try {
          if (!w || !w.document) throw new Error('blocked');
        } catch (e) {
          UI.log('Không truy cập được iframe core trên trình duyệt này.', 'error');
          return done(null);
        }
        ...
        try {
          w.addEventListener('error', function (ev) {
            try {
              UI.log('Lõi iframe ném lỗi: ' + (ev && ev.message ? ev.message : '?') + ' (dòng ' + (ev && ev.lineno ? ev.lineno : '?') + ')', 'error');
            } catch (e2) {}
          }, true);
        } catch (e) {}
        ...
        var doc = w.document;
        doc.open();
        doc.write('<!DOCTYPE html><html><head></head><body><script>' + SHIM_SCRIPT + '<\/script><script>' + buildSpoofScript() + '<\/script><script>' + SEAL_SCRIPT + '<\/script><script>' + cfgScript + '<\/script><script src="' + coreUrl + '"><\/script></body></html>');
        doc.close();
        ...
        function failoverEval() {
          ...
          safeRequest({
            ...
            onload: function (r2) {
              ...
              if (src.length < 200) {
                UI.log('Core live rỗng (' + src.length + 'B).', 'error');
                diagCore(w);
                return done(null);
              }
            ...
            onerror: function () {
              UI.log('Không tải được shortearn.live.js.', 'error');
              done(null);
            },
            ontimeout: function () {
              UI.log('Hết hạn tải core.', 'error');
              done(null);
            }
          });
        }
        
        pollCore(w, 40, function (ok) {
          if (ok) succeed();
          else failoverEval();
        });
      },
      onerror: function () {
        UI.log('Không với tới octolink.vip (kiểm tra mạng/VPN).', 'error');
        done(null);
      },
      ontimeout: function () {
        UI.log('Hết hạn jsconfig.', 'error');
        done(null);
      }
    });
  }
  ...
  function buildGuardHeaders(w, pathName, payload) {
    var headers = {};
    try {
      var gh = w.__hf(pathName, payload);
      for (var k in gh) headers[k] = String(gh[k]);
    } catch (e) {}
  ...
  function decryptJob(w, body) {
    ...
    try {
      if (w && w.Uint8Array) {
        wb = new w.Uint8Array(body.length);
        for (var i0 = 0; i0 < body.length; i0++) wb[i0] = body[i0];
      }
    } catch (e) {}
    try {
      if (typeof w.__se331 === 'function') w.__se331(wb);
      if (typeof w.__se4ce === 'function') job = w.__se4ce(wb);
    } catch (e) {}
    if (!job) {
      try {
    ...
    safeRequest({
      ...
      onload: function (resp) {
        ...
        if (cfg.rd) {
          ...
          UI.log('Mã hóa đã làm mới: ' + cfg.rd.slice(0, 16) + '…', 'success');
        } else {
          UI.log('Không thể làm mới mã hóa, dùng giá trị cũ.', 'warn');
        }
        try {
          if (cfg.nad != null) w.nad = cfg.nad;
          if (cfg.w1 != null) w.w1 = cfg.w1;
          if (cfg.w2 != null) w.w2 = cfg.w2;
          if (cfg.w3 != null) w.w3 = cfg.w3;
          if (cfg.ad != null) w.ad = cfg.ad;
          if (cfg.fk) w.fk = cfg.fk;
          if (cfg.fr != null) w.fr = cfg.fr;
          if (cfg.dm) w.dm = cfg.dm;
        } catch (e) {}
        done(cfg);
      },
      onerror: function () {
  ...
  function checkJob(targetUrl, attempt) {
    ...
    var w = coreCtx.w;
    if (attempt > 3) {
      UI.log('Tên miền lưu trữ đã hết hạn hoặc bị từ chối. Vui lòng cập nhật tên miền mới.', 'error');
      return;
    }
    refreshConfig(w, function () {
      ...
      if (!payload) {
        UI.log('Core không tạo được payload.', 'error');
        return;
      }
  ...
  function runStage(targetUrl, step) {
    ...
    var iv = setInterval(function () {
      ...
      if (left <= 0) {
        clearInterval(iv);
        sendContinue(targetUrl, step, 0, 0);
      }
    }, 1000);
  }
  ...
  function sendContinue(targetUrl, step, attempt, waitRound) {
    ...
    var w = coreCtx.w;
    if (attempt > 3) {
      UI.log('Phát sinh lỗi vượt chặng ' + step + '. Tạm dừng.', 'error');
      return;
    }
    if (waitRound > 6) {
      UI.log('Server chờ mãi ở chặng ' + step + '. Tạm dừng.', 'error');
      return;
    }
    ...
    if (!payload) {
      UI.log('Core không tạo được payload continue.', 'error');
      return;
    }
    ...
    postBinary('https://octolink.vip/check/continue', payload, headers, function (result) {
      ...
      if (!job) {
        ...
        return setTimeout(function () {
          sendContinue(targetUrl, step, attempt + 1, waitRound);
        }, 3000);
      }
      if (job.status === 'finish') {
        ...
        setTimeout(function () {
          sendContinue(targetUrl, step, attempt, waitRound + 1);
        }, delay);
      }
    }, function () {
  ...
  function main() {
    ...
    safeRequest({
      method: 'GET',
      url: 'https://api.github.com/repos/aozoracyrus/octolink-bypass',
      timeout: 10000,
      onload: function (r) {
        UI.log('Mạng OK → api.github.com (' + r.status + ', ' + (Date.now() - netT0) + 'ms)', 'detect');
      },
      onerror: function () {
        UI.log('api.github.com không với tới — bỏ qua.', 'warn');
      }
    });
    ...
    if (gMenu) {
      gMenu('Copy link finish gần nhất', function () {
        ...
        if (last && gClip) {
          ...
          notify('Chưa có link finish nào.');
        }
      });
    }
  } catch (e) {}
